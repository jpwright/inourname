...
$billURL = 'http://api.realtimecongress.org/api/v1/bills.xml?apikey='.$apikey.'&bill_id='.$bill_id.'&cosponsor_ids__in='.$bioguide_id.'&sections=short_title,official_title,last_action,enacted,summary,bill_type,number,session&page=1&per_page=8';
...
foreach($obj->bills->bill as $bill) {
	...
	echo '<div class="summary-legislation" id="'.$bioguide_id.'-'.$sumNum.'" onClick="expandSummary(\''.$bioguide_id.'-'.$sumNum.'\',\''.addslashes(str_replace("\"","''",str_replace("\n"," ",$summary))).'\',\''.addslashes(str_replace("\"","''",str_replace("\n"," ",$shortSummary))).'\')">';
